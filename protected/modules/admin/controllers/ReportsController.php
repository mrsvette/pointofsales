...
class ReportsController extends EController
{
	...
	public function actionExportExcel($date)
	{
		...
		$columns = array(
				...
				array(
					'header'=>Yii::t('order','Jml Item Siang'),
					'value'=>'Order::getCountOrderItemDate(date("Y-m-d",strtotime($data->date_entry)),$data->product_id,1)',
					'footer'=>Order::getCountOrderItemDate(date("Y-m-d",strtotime($date)),0,1), 
				),
				array(
					'header'=>Yii::t('order','Jml Item Malam'),
					'value'=>'Order::getCountOrderItemDate(date("Y-m-d",strtotime($data->date_entry)),$data->product_id,2)',
					'footer'=>Order::getCountOrderItemDate(date("Y-m-d",strtotime($date)),0,2), 
				),
				array(
				...
				array(
					'header'=>Yii::t('order','Jml Pendapatan Siang'),
					'value'=>'Order::getTotalOrderDate(date("Y-m-d",strtotime($data->date_entry)),$data->product_id,1)',
					'footer'=>Order::getTotalOrderDate(date("Y-m-d",strtotime($date)),0,1), 
				),
				array(
					'header'=>Yii::t('order','Jml Pendapatan Malam'),
					'value'=>'Order::getTotalOrderDate(date("Y-m-d",strtotime($data->date_entry)),$data->product_id,2)',
					'footer'=>Order::getTotalOrderDate(date("Y-m-d",strtotime($date)),0,2), 
				),
				array(
	...
	public function actionPush()
	{
		if(Yii::app()->request->isAjaxRequest ){
			...
			foreach($models as $model){
				$curl=$this->get_curl(
					'http://analitik.solusidatastatistik.com/appadmin/api/invoices',
					array(
